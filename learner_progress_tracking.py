...
# 10. Create 5 global functions captureMarks(), recursiveSum(), calculateAverage(), 
...
# 6. Create a base class of Person with two attributes of name and age
class Person:
...
# 1. Create a class called Learner with 5 starting attributes, learnerID, name, age and marks
class Learner(Person): # 7. Make Learner Inherit from base class Person
    def __init__(self, learner_id, name, age, course):
        # 8. Remove the previous attributets of name and age and replace them with the 
        ...
        # 9. Create Encapsulated values for average mark and learner status
        self.__average_mark = 0
        self.__learner_status = "Pass"
        
        # To use the encapsulated attributes we have to use methods 
    def getAverage(self):
    ...
    # 2. Create a new add learners method function 
    ...
    # 3. Create a view learners details method function 
    ...
    # 4. Create an update learner details methods function
    def updateLearnerDetails(self):
        # We ask the user to enter the ID of the learner details the want to update
        check_id = input("Enter learner ID to update: ")
        # Then we check if the entered id is == to the learner id if so we ask the user to enter 
        # the new details
        if check_id == self.learner_id:
    ...
    # 5. Create a remove learner methods function
...
while choice_input != "8": # Here we are saying if the user doesn't choose 8 we can 
    ...
    # Option 5 updates learner details 
        ...
        # We update learner details by calling the updateLearnerDetails function
        for get_learner in all_learners:
    ...
    # We display learner results only with option 7
        ...
        if get_learner is None:
            ...
            # Becuase we don't want to view all learner details we asign it to a value so we can 
            # capture the value we want
            get_learner.viewLearnerDetails()
            # Then we print values we want 
            print("")
            print("Result for: " + get_learner.name)
            print("Average: " + str(round(get_learner.getAverage(), 2)))
            print("Performance: " + get_learner.getStatus())
            print("")
            # We check if the specific learner gets a certificate 
